...
function StudentDiagnosis({ student, onMistakes }: { student: StudentSession; onMistakes: () => void }) {
  ...
  return <div className="content studentDiagnosisPage">
    ...
    <div className="studentPrivacyNote"><span>🔒</span><div><b>학생 전용 화면이에요.</b><p>학원 관리 정보는 보이지 않으며, 내가 저장한 문제와 학습 진단만 사용할 수 있어요.</p></div></div>
  </div>;
}
...
function Mistakes({ image, file, analyzed, fileInput, onImage, onAnalyze, canSubmit = false, studentName = "학생", onSaved }: { image: string | null; file: File | null; analyzed: boolean; fileInput: React.RefObject<HTMLInputElement | null>; onImage: (e: ChangeEvent<HTMLInputElement>) => void; onAnalyze: () => void; canSubmit?: boolean; studentName?: string; onSaved?: () => void }) {
  ...
  return <div className="content"><div className="pageTitle compact"><div><p>{canSubmit ? `${studentName}의 오답노트` : "문제 파일 분석"}</p><h1>문제를 찍거나 PDF로 올리면 <em>유형부터 풀이까지</em> 보여줘요.</h1><span>{canSubmit ? "이 페이지에서 분석한 문제와 메모를 내 오답노트에 저장해서 나중에 다시 볼 수 있어요." : "사진과 PDF에서 문제 유형과 예상 혼동 지점을 분석하고, 학생 풀이가 있으면 실제 오류 지점까지 찾아냅니다."}</span></div></div>
