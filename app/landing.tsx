...
const features = [
  { icon: "✎", title: "오답 분석", desc: "문제 사진이나 PDF를 올리면 유형과 단계별 풀이, 자주 헷갈리는 지점을 바로 보여줘요." },
  ...
  { icon: "▦", title: "학원 관리", desc: "반 평균, 취약 개념 히트맵, 학생별 진단을 한 화면에서 관리해요." },
];

const steps = [
  { n: "1", title: "문제 사진·PDF 업로드", desc: "학생이 헷갈린 문제를 사진으로 찍거나 PDF로 올려요." },
  ...
  return (
    <main className="landingPage">
      ...
      <section className="landingHero">
        <span className="landingBadge">학원·학부모를 위한 수학 오답 관리</span>
        <h1>오답 하나에서<br /><em>다음 학습 방향</em>까지</h1>
        <p>문제 사진 한 장으로 유형과 풀이를 분석하고, 학생별 학습 진단과 오답노트를 한곳에서 관리하세요.</p>
      ...
      <section className="landingAudience">
        <article><span>학생</span><h3>내 오답만 모아보는 학습실</h3><p>선생님이 등록한 이메일로 가입하면 자동으로 내 학습 페이지와 연결되고, 문제를 제출하고 나만의 오답노트를 관리할 수 있어요.</p></article>
        <article><span>선생님</span><h3>반 전체를 한 화면에서</h3><p>학생별 진단과 제출함, 반 평균과 취약 개념 히트맵까지 학원 관리 화면 하나로 확인하세요.</p></article>
      </section>
